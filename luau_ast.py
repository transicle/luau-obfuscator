...
class LocalDecl(Node):
    def __init__(self, names, exprs):    self.names = names;  self.exprs = exprs

class Assign(Node):
    def __init__(self, targets, exprs):  self.targets = targets; self.exprs = exprs
...
class IfStmt(Node):
    def __init__(self, clauses, else_body): self.clauses = clauses; self.else_body = else_body
    # clauses = [(cond, body), ...]
...
class CallStmt(Node):
    def __init__(self, call_expr):       self.call_expr = call_expr
...
class IndexExpr(Node):
    def __init__(self, obj, key):        self.obj = obj;  self.key = key
